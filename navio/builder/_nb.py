    ...
    if from_command_line or task not in completed_tasks:
        ...
        if task.ignored:
            ...
            try:
                ...
                stopTime = int(round(time.time() * 1000))
                logger.critical("Error in task \"%s\". Time: %s sec" % (task.name, (float(stopTime)-startTime)/1000))
                logger.critical("Aborting build")
                raise
            
            logger.info("Completed task \"%s\". Time: %s sec" % (task.name, (float(stopTime)-startTime)/1000))
